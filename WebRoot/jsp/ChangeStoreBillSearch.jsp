...
<table width="980" border="0" align="center" cellpadding="0" cellspacing="0">
  ...
  <tr> 
    <td width="6"></td>
    <td width="379" height="20" class="mainhead">转仓单查询</td>
...
<%
	...
	ChangeStoreBillForm  gif = (ChangeStoreBillForm )request.getAttribute("gif");
...
%>
    	...
    	<form name="form1" method="post" action="changeStoreBillSearch.do" >
	  ...
	  <tr>
            ...
            <td >
            	<SELECT name="fromStore" onChange="UpdateBill();">
            		...
            		%>
            		<option value="<%=tmpFact.getId()%>" <%=(tmpFact.getId()==gif.getFromStore()?"selected":"") %>><%=tmpFact.getName()%></option>
			<%
				}
           ...
           <td >
            	<SELECT name="toStore" onChange="UpdateBill();">
            		<option value="0">请选择...</option>
            		<%
            		            	for(int i=0; i<StoreList.size(); i++)
            		            	{
            		            		tmpFact = (StoreInfoForm)StoreList.get(i);
            		%>
            		<option value="<%=tmpFact.getId()%>" <%=(tmpFact.getId()==gif.getToStore()?"selected":"") %>><%=tmpFact.getName()%></option>
			<%
				}
...
%>
	...
	<TABLE  class="mailtable">  
		...
		<th><a class="tablehead" href="javascript:orderList('totalnum','<%=orderby%>');">重量(吨)</a><%if(orderby.equals("totalnum")) out.print("↓"); if(orderby.equals("totalnum desc")) out.print("↑");%></th>
		...
		ChangeStoreBillForm tmpInfo = null;
	    for (int index=0; index<ibsri.rslist.size(); index++)
		{
	      	tmpInfo = (ChangeStoreBillForm)ibsri.rslist.get(index);
	    %>
	        <tr <%if((index%2)==1){%>bgcolor="#CAE4F4"<%}%> >
	        	        <td align='center' width=3%><a href="javascript:viewDetail('<%= request.getContextPath() + "/changeStoreItemsView.do?billId=" + tmpInfo.getBillId()+"&deptid="+tmpInfo.getDeptid() %>');"><IMG src="images/news1.gif" border=0 alt='查看单据明细'></a></td>
