...
<table width="980" border="0" align="center" cellpadding="0" cellspacing="0">
  ...
  <tr> 
    <td width="6"></td>
    <td width="379" height="20" class="mainhead">二次加工单查询</td>
...
<%
	...
	KaiPingForm  gif = (KaiPingForm )request.getAttribute("gif");
...
%>
    	...
    	<form name="form1" method="post" action="kaiPingSearch.do" >
	  ...
	  <tr>
            ...
            <td >
            	<INPUT class=none  name="fromGoodsId" type="text" size="20" value="<%=gif.getFromGoodsId() %>">
            </td>
             <td  align="right" > 
                <b>类型:</b>
            </td>
           <td >
            	<select name="kind">
            		<option value=0>全部</option>
            		<option value=-1 <%=(gif.getKind()==-1?"selected":"") %>>原料</option>
            		<option value=1 <%=(gif.getKind()==1?"selected":"") %>>成品</option>
            	</select>
            </td>
            <td  align="right"> 
                <b>录入员:</b>
...
<%
	...
	{
%>
	<font color=#215385>单据列表（共 <b><%=ibsri.allnum%></b> 条记录，合计原料 <b><%=ibsri.allsumnum%></b> 吨 ，成品 <b><%=ibsri.oddmoney %></b>吨，金额 <b><%=nf.format(ibsri.allmoney)%></b> 元）</font>
	...
	<TABLE width="100%" border="1" cellpadding="3" cellspacing="0" bordercolor="#FFFFFF" class="mailtable">  
		...
		<th><a class="tablehead" href="javascript:orderList('goodsid','<%=orderby%>');">商品编号</a><%if(orderby.equals("goodsid")) out.print("↓"); if(orderby.equals("goodsid desc")) out.print("↑");%></th>
		<th>商品名称</th>
		<th>商品规格</th>
		<th><a class="tablehead" href="javascript:orderList('num','<%=orderby%>');">个数</a><%if(orderby.equals("num")) out.print("↓"); if(orderby.equals("num desc")) out.print("↑");%></th>
		<th><a class="tablehead" href="javascript:orderList('amount','<%=orderby%>');">重量(吨)</a><%if(orderby.equals("amount")) out.print("↓"); if(orderby.equals("amount desc")) out.print("↑");%></th>
		<th><a class="tablehead" href="javascript:orderList('jine','<%=orderby%>');">金额(元)</a><%if(orderby.equals("jine")) out.print("↓"); if(orderby.equals("jine desc")) out.print("↑");%></th>		
		<th><a class="tablehead" href="javascript:orderList('createtime','<%=orderby%>');">录入时间</a><%if(orderby.equals("createtime")) out.print("↓"); if(orderby.equals("createtime desc")) out.print("↑");%></th>
		</tr>
		<%
			}
		%>
		...
		{
	      	tmpInfo = (KaiPingForm)ibsri.rslist.get(index);
	    %>
	        <tr <%if((index%2)==1){%>bgcolor="#CAE4F4"<%}%> >
	        <td align='center' width=3%><a href="javascript:viewDetail('<%= request.getContextPath() + "/kaiPingItemsView.do?billId=" + tmpInfo.getBillid()+"&deptid="+tmpInfo.getDeptid() %>');"><IMG src="images/news1.gif" border=0 alt='查看单据明细'></a></td>
	        ...
	        <td align='center' ><%= tmpInfo.getFromGoodsName() %></td>
	        <td align='center' ><%= tmpInfo.getFromGuige() %></td>
	        <td align='right' ><%= tmpInfo.getFromNum()  %><%= tmpInfo.getDanwei()%></td>
	        <td align='right' ><%= tmpInfo.getFromAmount()  %></td>
			<td align='right'><%= tmpInfo.getJine() %></td>
