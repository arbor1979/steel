...
<table width="980" border="0" align="center" cellpadding="0" cellspacing="0">
  ...
  <tr> 
    <td width="6"></td>
    <td width="379" height="20" class="mainhead">月利润变化表</td>
...
<%
	...
	for(int i=0;i<tmp3.length;i++)
		total3=total3+tmp2[i]+tmp3[i];
%>
     <form name="form1" method="post" action="monthLiRun.do" onsubmit="return mySubmit1();">
	...
	<TABLE width="100%" border="1" cellpadding="3" cellspacing="0" bordercolor="#FFFFFF" class="mailtable">  
		...
		<%
		    for (int i=0; i<tmp1.length; i++)
		    	out.print("<td align=right>"+nf.format(tmp1[i])+"</td>");
		%>
			<td align=right><%=nf.format(total1) %></td>
		</tr>
		<tr>
			<td align=center>毛利</td>
		<%
		    for (int i=0; i<tmp3.length; i++)
		    	out.print("<td align=right>"+nf.format(tmp2[i])+"</td>");
		%>
			<td align=right><%=nf.format(total2) %></td>
		</tr>
		<tr>
			<td align=center>净利润</td>
		<%
		    for (int i=0; i<tmp3.length; i++)
		    	out.print("<td align=right>"+nf.format(tmp2[i]+tmp3[i])+"</td>");
		%>
			<td align=right><%=nf.format(total3) %></td>
		</tr>
	</TABLE>
	<img src="chart/<%=filename1 %>"><br>
