...
<body >
<%
	List ibf = (List)request.getAttribute("billInfo");
	String ifprint = (String)request.getAttribute("ifprint");
	NumberFormat nf = NumberFormat.getNumberInstance();
	nf.setMaximumFractionDigits(2);		
	nf.setMinimumFractionDigits(2);
	AccountForm af=(AccountForm)ibf.get(0);
%>
<br>
<h2 align="center" style="font-family: 楷体_GB2312;"><%=af.getShortname() %>【存取款单】</h2>
...
<tr >
	...
	<table width="100%" align="center" bordercolor="#000000" border=1 cellpadding="5" cellspacing="0" style="border-collapse:collapse">
		...
		<%
		for(int i=0;i<ibf.size();i++)
		{
			af=(AccountForm)ibf.get(i);
		%>
		<tr bgcolor="#ffffff">
...
<%  
	if(ifprint.equals("1"))
		out.print("window.location='accessMoney.do';");
	else
		out.print("window.close();");
%>
</script>
<%}else{ %>
<p align="center"><input type="button" name="printtr" value=" 打印 " onclick="window.location='accessMoneyItemView.do?billId=<%=af.getBillid() %>&deptid=<%=af.getDeptid() %>&ifprint=2';">
