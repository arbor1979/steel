...
<%} %>
	...
	<table width="100%" align="center" bordercolor="#000000" border=1 cellpadding="3" cellspacing="0" style="border-collapse:collapse">
		...
		<tr bgcolor="#ffffff">
			...
			<td><b>大写</b></td><td colspan="4" align="left" ><%chineseMoney cm=new chineseMoney();out.print(cm.getChineseMoney(ibf.getTotalPrice()*ibf.getKind()+ibf.getTotaljiagong())); %></td>
...
<%} %>
	...
	<tr><td colspan=2><%=ibf.getTip() %></td><td height=30 align=right colspan=2><u><i>天天阳光软件</i></u></td></tr>
	<%} %>
	</table>
	<%
...
<%}else{ %>
<p align="center"><input type="button" name="printtr" value=" 打印 " onClick="window.navigate('exportItemsView.do?billId=<%=ibf.getBillId() %>&deptid=<%=ibf.getDeptid() %>&ifprint=2');">
